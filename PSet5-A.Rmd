...
tech_job %>%
  ...
  coord_flip()
```
>>>>>>> cdb3af3b1b1bd4ab4d5f85c673842a162223bd23
...
```{r}
library(leaflet)
job = read_csv('data/raw/NYC_Jobs.csv')%>%
  filter(!is.na(latitude)) %>%
  filter(longitude < -74.0060 + 1) %>%
  filter(longitude > -74.0060 - 1) %>%
  filter(latitude < 40.7128 + 0.8) %>%
...
ui <- bootstrapPage(
  ...
  absolutePanel(top = 10, left = 50,
                ...
                # numericInput("room_night", "Number of nights to stay", value = 5),
                # checkboxInput("is_superhost", "Has to be a superhost", FALSE)
  )
)
...
server <- function(input, output, session) {
  ...
  filteredData <- reactive({
    ...
    #   temp <- temp[temp$room_type == input$room_type,]
    # }
    temp[#temp$host_is_superhost == input$is_superhost &
           temp$Annual_salary >= input$Annual_salary[1] & temp$Annual_salary <= input$Annual_salary[2] 
        #   temp$accommodates >= input$room_accomodate[1] & temp$accommodates <= input$room_accomodate[2] &
           ,]#temp$minimum_nights <= input$room_night & temp$maximum_nights >= input$room_night,] 
  })
  ...
  output$map <- renderLeaflet({
    leaflet() %>% setView(lat = 40.7128, lng = -74.0060, zoom = 11) %>%
      ...
      addCircleMarkers(data = filteredData(),  
                       ...
                       radius = 3, 
                       color = ~color,
                       label = ~paste(Job.Category, Annual_salary))
  })
}
