...
As mentioned above, we fill all empty slots in the original dataset with NAs.

After drawing a heatmap to show the missing patterns using `mi::missing_data.frame()`, we receive a warning message saying that all values in column `Recruiment Contact` are NAs. Therefore, we simply drop this column from our data frame. For other columns, we will keep these NAs for now.
...
(mini_freq, 20)
htmlTable(
  (mini_freq, 20), caption="Minimum Qual Requirements in All Jobs Word Frequency", 
...
(pref_freq, 20)
htmlTable(
  (pref_freq, 20), caption="Preferred Skills in All Jobs Word Frequency", 
          er=c("Word", "Frequency"), rnames=FALSE)
```
```{r}
library(wordcloud2)
...
(tech_mini_freq, 20)
htmlTable(
  (tech_mini_freq, 20), caption="Minimum Qual Requirements in Technology Related Jobs Word Frequency", 
          er=c("Word", "Frequency"), rnames=FALSE)
```
```{r}
library(wordcloud2)
...
(tech_pref_freq, 20)
htmlTable(
  (tech_pref_freq, 20), caption="Preferred Skills in Technology Related Jobs Word Frequency", 
          er=c("Word", "Frequency"), rnames=FALSE)
```
```{r}
library(wordcloud2)
...
# 
# main_salary<-salary_geocodes%>%
#     filter(Annual_salary >  quantile(salary_geocodes$Annual_salary,0.05))%>%
#     filter(Annual_salary < quantile(salary_geocodes$Annual_salary,0.95))
# 
# main_salary<-main_salary[order(main_salary$Annual_salary),]
# 
# main_salary %>% 
#   leaflet() %>% 
...
#                                   "<br /><strong>Annual Salary:</strong>","$",main_salary$Annual_salary,
...
summary(job$Annual_salary_range)
```
...
for (i in 1:110){cate_freq_mean$Annual_salary_mean[i] <- mean(mydf$salary_annual_mean[mydf$Job.Category == cate_freq_mean$Var1[i]])}
cate_freq_mean$cate_sala_max <- NA
for (i in 1:110){cate_freq_mean$cate_sala_max[i] <- max(mydf$salary_annual_mean[mydf$Job.Category == cate_freq_mean$Var1[i]])}
cate_freq_mean$cate_sala_min <- NA
for (i in 1:110){cate_freq_mean$cate_sala_min[i] <- min(mydf$salary_annual_mean[mydf$Job.Category == cate_freq_mean$Var1[i]])}
