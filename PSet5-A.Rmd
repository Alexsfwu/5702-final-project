...
ggplot(categoryList, aes(fct_relevel(fct_rev(fct_infreq(fct_explicit_na(Category, "NA"))), "NA"))) +
...
3. For salaries calculated daily, there is no specific pattern regarding the distribution. Some jobs have a relatively low daily salaries, while others have a much higher salaries.
4. For salaries calculated hourly, most of them has a relatively low value, but there are still some jobs have a relatively high hourly salaries.
...
In conclusion, these are all we have done right now. We identify the number of job postings with respect to different job categories. We also check the salaries of jobs with different salari frequency. Last but not least, we also inspect the salary range for different categories by drawing a boxplot. I think we are off a great start, and we will continue our analysis on the dataset from here.