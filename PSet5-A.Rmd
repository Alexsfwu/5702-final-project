...
ggplot(df_popular, aes(x=month,y=fct_reorder(Job.Category,count),fill=Job.Category))+
  ...
  theme(plot.title = element_text(hjust = 0.5))
```
The plot above showed us the change in the number of posting of popular categories from January to December. As we can see most of the job posting posted between August to December, which makes sense because most recruiting season happens during the fall. We also can tell some of the job categories has demand in other seasons. For example, Public Safty, Inspections and Enforcement also have some recruiting demand in May. Some of the job categories have stable recruiting demand around all year long, like Maintenance, Architecture, and Engineering.
...
theme(plot.title = element_text(hjust = 0.5)) +
  coord_flip()
```
This boxplot gives us a general idea of the salary distribution for different kinds of jobs from the highest salary to lowest mean salary. For instance, we can see that jobs of Building Operations & Maintenance, in general, have lower salaries than those of Information Technology & Telecommunications.We also can have a general idea that the higher the Annual Salary the wider the range of the Salary. We will explore more on the topic latter.


### 5.3 Word Clouds for Text Information

Meanwhile, we also want to study the minimum qualification requirements and preferred skills for available jobs in NYC. We want to find if there are any patterns in these two columns and if we can extract any useful information from them. To illustrate our findings graphically, we decide to use Word Clouds to show the most frequent words in these texts.

So what is Word Clouds? Word Clouds is a visual representation of text data. They are useful for quickly perceiving the most prominent terms, which makes them widely used in media and well understood by the public. A Word Cloud is a collection of words depicted in different sizes. The bigger and bolder the word appears, the greater the frequency within a given text and the more important it is.

To extract meaningful vocabularies from the text descriptions, we take advantage of the text mining package `tm` in R. This package is based on the ideas of Natural Language Processing (NLP). It has methods that can transform all words to lowercase, remove words that are uninformative in Enlight such as "a" and "the", and get rid of whitespaces and punctuations.
...
>>>>>>> 00785b810378c93d0199733ea6ee82d716bb017f
```{r}
library(wordcloud2)
...
This table shows the 20 most frequent words in Preferred Skills among all jobs in out dataset.
```{r}
...
library(webshot)
webshot::install_phantomjs(force = TRUE)
pref_freq_graph <- wordcloud2(data=pref_freq, color='random-light', backgroundColor='black')
library("htmlwidgets")
saveWidget(pref_freq_graph,"pref_freq_graph.html", selfcontained = F)
```
...
This table shows the 20 most frequent words in Minimum Qual Requirements among tech jobs in out dataset.
```{r}
...
library(webshot)
webshot::install_phantomjs(force = TRUE)
tech_mini_freq_graph <- wordcloud2(data=tech_mini_freq, color='random-light', backgroundColor='black')
library("htmlwidgets")
saveWidget(tech_mini_freq_graph,"tech_mini_freq_graph.html", selfcontained = F)
```

This table shows the 20 most frequent words in Preferred Skills among tech jobs in out dataset.
```{r}
...
library(webshot)
webshot::install_phantomjs(force = TRUE)
tech_pref_freq_graph <- wordcloud2(data=tech_pref_freq, color='random-light', backgroundColor='black')
library("htmlwidgets")
saveWidget(tech_pref_freq_graph,"tech_pref_freq_graph.html", selfcontained = F)
```
...
tech_job = job[job$Job.Category == 'Technology, Data & Innovation', ]%>%
  ...
  mutate(Full.Time.Part.Time.indicator = if_else(is.na(Full.Time.Part.Time.indicator), "Dosen't mention", Full.Time.Part.Time.indicator))

tech_job %>%
  #filter(!is.na(Full.Time.Part.Time.indicator)) %>%
  #mutate_at(Full.Time.Part.Time.indicator, ~replace(., is.na(.),"Dosn't mention" ))
...
```
From the above plot we can see 79% of the posted job in Technology category are Full time job. Only 5% of the job are part time job. Rest of the posting didn't mention they are part time or full time job.
...
```
From the above plot we can see 94% of the posted job in Technology category posting Anuual salat. Only 5% of the job posting Hourly salary. Rest of the posting Posting Daily salary.
...
tech_job %>%
  ...
  coord_flip()
```
To take a closer view, we looked into the Job Count w.r.t Civil Service Title. From the polt above, we can see in the category of Technology, Computer system manager is the most frequent title.It was posted more than 50 times in our dataset. It is almost the double count of the second one: Computer specialist(software). The fewest title is supervising computer service. It only was posted 2 times in our dataset.
...
tech_job %>%
  ...
  coord_flip()
```
we looke at the Job Count w.r.t Location. From the plot above, we can see in the category of Technology, most of the jobs are in 255 greenwich street, 2 metro tech and 355 adam street. From a map we can see the first 10 locations are clustered. We can tell the technology jobs are very location sensitive. If you are willing to relocate, you can have more opportunities.  The location have fewest job posting is 109 E 16th st, 11 metrotech center , 356 flushing ave and 9 metrotech center.
Later we have a interactive map, so we can have more insight on location.
...
```
Th pot above shows the annual salary for different cicil title. The annual salary range form $ 40000 to $120000. The highest one is administrative business promot with arround $11500 annual salary and the lowest is college aid with 40000 annual salary.



## 5.5 Explore the relationship between salary and its range
...
```{r}
ggplot(mydf1, aes(x=AnnualSalaryMean, y=AnnualSalaryRange, color=type)) + geom_point(shape=1)
```
...
```{r eval=FALSE, include=FALSE}
hPlot(Annual_salary_range~Annual_salary_mean,data = top_popular_cate,type = 'bubble', group = "Var1", size = 'Freq',title = 'Relationship between annual salary range and amount based on top 30 popular job categories',subtitle = "(categories can be filtered by click the legend)")

```

[click here to open the plot](data/bubbleplot1.html)
...
```{r eval=FALSE, include=FALSE}
hPlot(Annual_salary_range~Annual_salary_mean,data = cate_freq_mean,type = 'bubble', group = "Var1", size = 'Freq',title = 'Relationship between annual salary range and amount based on job categories',subtitle = "(categories can be filtered by click the legend)")
```

[click here to open the plot](data/bubbleplot2.html)
...
#             colors = c('red', 'blue'), 
#             labels = c('High salary', 'Low salary'),
#             title = "Salary",
#     opacity = 1
#   )
#   })
...
## Summary of Studies on the Dataset
...
Detailed results can be found in the previous chapters.
